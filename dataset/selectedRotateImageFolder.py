...
class SelectedRotateImageFolder(datasets.ImageFolder):
    def __init__(self, root, train_transform, original=True, rotation=True, rotation_transform=None, label_shifts=False):
        ...
        if not label_shifts:
            random.shuffle(self.samples) # shuffle
...
def prepare_test_data(args, use_transforms=True):
    if args.corruption == 'original' or args.corruption == 'rendition' or args.corruption == 'v2' or args.corruption == 'sketch' or args.corruption == 'adversial':
    ...
    if not hasattr(args, 'corruption') or args.corruption == 'original':
        ...
        teset = SelectedRotateImageFolder(validdir, te_transforms_local, original=False, rotation=False,
                                          ...
                                          label_shifts= args.exp_type=='label_shifts')
    ...
    if not args.corruption in common_corruptions:
        # other dataset are for single-domain TTA only
