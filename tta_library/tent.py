...
class Tent(nn.Module):
    ...
    def __init__(self, model, optimizer, fishers=None, training_avg=None, training_var=None, steps=1, episodic=False,
                 ensemble_weights = None, ema_weights = None):
        super().__init__()
        self.model = model
        ...
        if ensemble_weights is not None:
            self.source_model = copy_model_only(self.model)
    ...
    def ensembling(self):
        if self.ensemble_weights is not None:
            for param, source_param in zip(self.model.parameters(), self.source_model.parameters()):
                param.data[:] = self.ensemble_weights*param[:].data[:] + (1-self.ensemble_weights)*source_param[:].data[:]
