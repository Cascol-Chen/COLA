...
class EATA(nn.Module):
    ...
    """
    def __init__(self, model, optimizer, fishers=None, fisher_alpha=2000.0, steps=1, episodic=False, e_margin=math.log(1000)*0.40, d_margin=0.05):
        super().__init__()
        self.model = model
    ...
    def forward(self, x, y=None):
        ...
        if self.steps > 0:
            for _ in range(self.steps):
                outputs, updated_probs = forward_and_adapt_eata(x, self.model, self.optimizer, self.fishers, 
                                                                self.e_margin, self.current_model_probs, 
                                                                fisher_alpha=self.fisher_alpha,
                                                                d_margin=self.d_margin, imagenet_mask=self.imagenet_mask)
    ...
    def ensembling(self):
        if self.ensemble_weights is not None:
            for param, source_param in zip(self.model.parameters(),
                                        self.source_model.parameters()):
                param.data[:] = self.ensemble_weights*param[:].data[:] + \
                    (1-self.ensemble_weights)*source_param[:].data[:]
        elif self.ema_weights is not None:
            for param, pre_param in zip(self.model.parameters(), self.pre_model.parameters()):
                param.data[:] = self.ema_weights*param[:].data[:] + (1-self.ema_weights)*pre_param[:].data[:]
            self.pre_model = copy_model_only(self.model)
...
def forward_and_adapt_eata(x, model, optimizer, fishers, e_margin, current_model_probs, fisher_alpha=50.0, d_margin=0.05, imagenet_mask=None):
...
def copy_model_and_optimizer(model, optimizer):
    ...
    # model_state = None # do not consider reset yet
