...
def get_agent(args, net, weight_paths):
    ...
    if args.algorithm == 'eta':
        ...
        net = CoLAViT(net, 0.01, logger=logger)
        net = eata.configure_model(net)
...
if __name__ == '__main__':
    ...
    if args.ensemble_weights is not None:
        algorithm_name += "_ew"
    elif args.ema_weights is not None:
        algorithm_name += "_ema"
    ...
    if not os.path.exists(args.output):
        ...
        os.makedirs(args.output+'model/', exist_ok=True)
    ...
    noises = ['gaussian_noise', 'shot_noise', 'impulse_noise']
    blurs = ['defocus_blur', 'glass_blur', 'motion_blur', 'zoom_blur']
    weathers = ['snow', 'frost', 'fog', 'brightness']
    digitals = ['contrast', 'elastic_transform', 'pixelate', 'jpeg_compression']

    net = net.cuda()
    
    corruptions = [
        [noises, blurs, weathers, digitals], # agent 1
        [blurs, noises, digitals, weathers], # agent 2
        [weathers, digitals, blurs, noises]  # agent 3
    ]

    logger.info('=== loading all test loaders ===')
    agent_loaders = []
    for agent_corruptions in corruptions:
        agent_loader = []
        for type_corruption in agent_corruptions:
            loader = []
            for corruption in type_corruption:
                args.corruption = corruption
                _, val_loader = prepare_test_data(args)
                loader.append(val_loader)
            agent_loader.append(loader)
        agent_loaders.append(agent_loader)

    agents = [get_agent(args, net, weight_paths) for _ in range(len(corruptions))]
    logger.info(args)
    logger.info(corruptions)

    community = Community(agents, agent_loaders, args.algorithm, corruptions, args.output + 'model/', logger, save_vectors_to_file_on=True)
    community.run_exp()
    
    logger.info(community.results)
    for i, agent in enumerate(community.results):
        agent_accs = agent['acc1']
        final_accs = [sum(acc)/len(acc) for acc in agent_accs]
        logger.info(f'agent {i} acc: {final_accs}')