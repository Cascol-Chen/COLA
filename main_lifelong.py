...
def validate_adapt(val_loader, model, args):
    ...
    with torch.no_grad():
        ...
        for i, dl in enumerate(val_loader):
            ...
            # For debug purpose, but I'm sure it can also offer some insights 
            if 'cola' in args.algorithm and i > 0 and i % 100 == 0:
                logger.info(model.model.plot_alpha())
...
def get_args():
    ...
    parser.add_argument('--seed', default=2022, type=int, help='seed for initializing training. ')
...
if __name__ == '__main__':
    ...
    if args.algorithm == 'tent':
        ...
        # Try this if domain detection is too difficult in your case 
        # net = CoLAViT(net, 0.01, logger=logger, domain_detect_on=False, save_per_iteration=100)
        net = eata.configure_model(net)
        ...
        eps_optimizer = torch.optim.SGD(params['epsilon_weight'] + params['epsilon_bias'], 0.001, momentum=0.9, weight_decay=0.4)
        ...
        base_optimizer = torch.optim.SGD
        optimizer = f_sam.SAM(params['epsilon_weight'] + params['epsilon_bias'], base_optimizer, alpha_optimizer, len(weight_paths), lr=0.001, momentum=0.9, weight_decay=0.1)
        net.cola_optimizer = optimizer
        adapt_model = sar.SAR(net, optimizer)
